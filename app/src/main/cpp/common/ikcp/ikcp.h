...
struct IKCPCB {
  ...
  // 流量统计相关变量
  IUINT32 tx_bytes_per_sec;    // 每秒发送的字节数
  IUINT32 rx_bytes_per_sec;    // 每秒接收的字节数
  IUINT32 tx_bytes_last;       // 上一次统计时的发送字节总数
  IUINT32 rx_bytes_last;       // 上一次统计时的接收字节总数
  IUINT32 tx_bytes_total;      // 发送的字节总数
  IUINT32 rx_bytes_total;      // 接收的字节总数
  IUINT32 traffic_time_start;  // 流量统计周期的开始时间

  IUINT32 loss_period;      // 丢包率计算周期（毫秒）
  IUINT32 loss_time_start;  // 统计周期起始时间
  IUINT32 period_sent;      // 周期内发送的数据包数量
  IUINT32 period_retrans;   // 周期内重传的数据包数量
  float current_loss_rate;  // 当前丢包率（百分比）
...
// 获取平滑的rtt
IUINT32 ikcp_getsrtt(const ikcpcb *kcp);

// 获取当前丢包率（百分比）
float ikcp_get_lossrate(const ikcpcb *kcp);

// 设置丢包率计算的周期（毫秒）
void ikcp_set_loss_period(ikcpcb *kcp, IUINT32 period_ms);

// 获取接收速率
IUINT32 ikcp_get_rx_rate(const ikcpcb *kcp);

// 获取发送速率
IUINT32 ikcp_get_tx_rate(const ikcpcb *kcp);
