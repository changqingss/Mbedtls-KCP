 ...
 *			for code compactness, (at the expense of execution
 *speed).
 */
 ...
 */
void MD5Init(context) MD5_CONTEXT *context; /* context */
{
...
/* MD5 block update operation. Continues an MD5 message-digest
  ...
  context.
 */
void MD5Update(context, input, inputLen) MD5_CONTEXT *context; /* context */
 ...
 */
static void MD5Transform(state, block) unsigned int state[4];
...
/* Decodes input (unsigned char) into output (unsigned int). Assumes len is
  a multiple of 4.
 */
static void Decode(output, input, len) unsigned int *output;
unsigned char *input;
unsigned int len;
{
  unsigned int i, j;
...
static void MD5_memcpy(output, input, len) unsigned char *output;
unsigned char *input;
unsigned int len;
{
  unsigned int i;
 ...
 */
static void MD5_memset(output, value, len) unsigned char *output;
int value;
unsigned int len;
{
  unsigned int i;
...
int COMM_API_calculate_md5(const char *m_filename, char *m_md5, int md5Len) {
  ...
  while ((bytesRead = fread(data, 1, sizeof(data), file)) > 0) {
    MD5Update(&md5Context, data, bytesRead);
  }
  ...
  // 将 MD5 哈希值赋值给 m_md5
  // 确保 m_md5 的长度足够存储 32 个字符（每个字节转换为 2 位十六进制）和一个终止符 '\0'
  if (md5Len < 33) {  // 32 characters + 1 null terminator
    printf("m_md5 size is not enough\n");
    fclose(file);
    return -1;
  }

  for (int i = 0; i < 16; i++) {
    sprintf(&m_md5[i * 2], "%02hhx", digest[i]);
  }
