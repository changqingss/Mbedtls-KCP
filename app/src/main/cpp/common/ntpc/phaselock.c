...
double ntp_min_delay = 800.0; /* global, user-changeable, units are microseconds */
...
static int search(int rp, int j, int s0, int s1, int sign, struct _seg *answer) {
  ...
  for (n = next_up(j); n != next_up(rp); n = next_up(n)) {
    if (0 && debug)
      NTP_INFO("d_ring[%d].s.ss[%d]=%f d_ring[%d].s.ss[%d]=%f", n, s0, d_ring[n].s.ss[s0], j, s1, d_ring[j].s.ss[s1]);
    dt = d_ring[n].absolute - d_ring[j].absolute;
    slope = (d_ring[n].s.ss[s0] - d_ring[j].s.ss[s1]) / dt;
    if (0 && debug) NTP_INFO("slope %d%d%d [%d,%d] = %f", s0, s1, sign, j, n, slope);
...
static double find_shift(double slope, double offset) {
  ...
  if (debug) NTP_INFO("find_shift %f %f -> %f", slope, offset, shift);
...
static void polygon_point(struct _seg *s) {
  double l, r;
  if (debug) NTP_INFO("loop %f %f", s->slope, s->offset);
  l = find_shift(-s->slope, s->offset);
  r = find_shift(s->slope, -s->offset);
  if (l < df.l_min) df.l_min = l;
  if (r < df.r_min) df.r_min = r;
  if (debug) NTP_INFO("constraint left:  %f %f", l, df.l_min);
  if (debug) NTP_INFO("constraint right: %f %f", r, df.r_min);
}
...
static double find_df_center(struct _seg *min, struct _seg *max, double gross_df) {
  ...
  if (debug) NTP_INFO("find_df_center %f %f", delta, factor);
  return factor * delta;
}
...
int contemplate_data(unsigned int absolute, double skew, double errorbar, int freq) {
  ...
  if (debug) NTP_INFO("xontemplate %u %.1f %.1f %d", absolute, skew, errorbar, freq);
  d_ring[rp].absolute = absolute;
  d_ring[rp].skew = skew;
  d_ring[rp].errorbar = errorbar - ntp_min_delay; /* quick hack to speed things up */
  d_ring[rp].freq = freq;
  ...
  if (valid == RING_SIZE) {
    ...
    for (j = rp;; j = n) {
      ...
      if (debug)
        NTP_INFO("hist %d %d %f %f %f", j, d_ring[j].absolute - absolute, cum, d_ring[j].s.s.min, d_ring[j].s.s.max);
      n = next_dn(j);
      if (n == rp) break;
    ...
    for (c = 1, j = next_up(rp);; j = nextj) {
      ...
      if (debug) NTP_INFO("maxseg[%d] = %f *x+ %f", c, maxseg[c].slope, maxseg[c].offset);
      last_slope = maxseg[c].slope;
      c++;
      if (nextj == rp) break;
    }
    if (dinit == 1) inconsistent = 1;
    if (debug && dinit == 0) NTP_INFO("mincross %f *x+ %f", save_min.slope, save_min.offset);
    max_avail = c;
    /*
    ...
    for (c = 1, j = next_up(rp);; j = nextj) {
      ...
      if (debug) NTP_INFO("minseg[%d] = %f *x+ %f", c, minseg[c].slope, minseg[c].offset);
      last_slope = minseg[c].slope;
      c++;
      if (nextj == rp) break;
    }
    if (dinit == 1) inconsistent = 1;
    if (debug && dinit == 0) NTP_INFO("maxcross %f *x+ %f", save_max.slope, save_max.offset);
    min_avail = c;
    /*
    ...
    if (!inconsistent) {
      ...
      for (dinit = 1, c = 1; c < max_avail; c++) {
        ...
        if (maxseg[c].slope > save_max.slope) break;
        if (dinit == 0) polygon_point(&maxseg[c]);
      }
      if (dinit && debug) NTP_INFO("found maxseg vs. save_min inconsistency");
      ...
      for (dinit = 1, c = 1; c < min_avail; c++) {
        ...
        if (minseg[c].slope < save_min.slope) break;
        if (dinit == 0) polygon_point(&minseg[c]);
      }
      if (dinit && debug) NTP_INFO("found minseg vs. save_max inconsistency");
      if (dinit) inconsistent = 1;
      min_imax = c;
    ...
    if (inconsistent) {
      NTP_INFO("# inconsistent");
    } else {
      delta_f = find_df(&both_sides_now);
      if (debug) NTP_INFO("find_df() = %e", delta_f);
      delta_f += find_df_center(&save_min, &save_max, delta_f);
      delta_freq = delta_f * 65536 + .5;
      if (debug) NTP_INFO("delta_f %f  delta_freq %d  bsn %d", delta_f, delta_freq, both_sides_now);
      computed_freq -= delta_freq;
      NTP_INFO("# box [( %.3f , %.1f ) ", save_min.slope, save_min.offset);
      NTP_INFO(" ( %.3f , %.1f )] ", save_max.slope, save_max.offset);
      NTP_INFO(" delta_f %.3f  computed_freq %d", delta_f, computed_freq);
